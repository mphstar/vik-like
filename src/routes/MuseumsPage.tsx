...
export function MuseumsPage(){
  const sections: SectionData[] = useMemo(()=> MUSEUMS.map(m => ({
    id: m.id,
    title: m.name,
    navLabel: m.name.split(/\s+/).slice(0,2).join(' '),
    subtitle: m.location,
    bg: m.hero,
    content: <p className="max-w-xl text-white/90 md:text-lg leading-relaxed">{m.description}</p>,
    ctaHref: `/museum/${m.id}`
  })),[]);

  // Preloader (copy pattern from App)
  ...
  useEffect(()=>{ if(!ready) return; const el=containerRef.current; if(!el) return; let startY=0,lastY=0,startTime=0,moved=false,gestureHandled=false; const cooldown=480; let lastTrigger=0; const onTouchStart=(e:TouchEvent)=>{ if(isAnimatingRef.current) return; if(e.touches.length!==1) return; startY=lastY=e.touches[0].clientY; startTime=performance.now(); moved=false; gestureHandled=false; }; const onTouchMove=(e:TouchEvent)=>{ if(e.touches.length!==1) return; const y=e.touches[0].clientY; if(Math.abs(y-startY)>6) moved=true; lastY=y; }; const onTouchEnd=(e:TouchEvent)=>{ if(!moved||gestureHandled) return; const now=performance.now(); if(now-lastTrigger<cooldown) return; const deltaY=lastY-startY; const abs=Math.abs(deltaY); const dur=now-startTime; const velocity=abs/Math.max(dur,1); const isFlick=velocity>1||(dur<180&&abs>55); const distanceOk=abs>(isFlick?35:90); if(!distanceOk) return; lastTrigger=now; gestureHandled=true; let next=active+(deltaY<0?1:-1); if(next<0) next=0; else if(next>=sections.length) next=sections.length-1; if(next!==active){ e.preventDefault(); scrollToIndex(next,{overshoot:!isFlick}); } }; el.addEventListener('touchstart',onTouchStart,{passive:true}); el.addEventListener('touchmove',onTouchMove,{passive:true}); el.addEventListener('touchend',onTouchEnd,{passive:false}); el.addEventListener('touchcancel',onTouchEnd,{passive:true}); return ()=>{ el.removeEventListener('touchstart',onTouchStart as any); el.removeEventListener('touchmove',onTouchMove as any); el.removeEventListener('touchend',onTouchEnd as any); el.removeEventListener('touchcancel',onTouchEnd as any); }; },[active,ready,sections.length]);

  if(!ready){
    return <div className="h-screen w-screen flex items-center justify-center bg-black text-white relative overflow-hidden" aria-busy="true" aria-label="Memuat aset museum">
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 px-6">
        <div className="flex items-center gap-4"><Logo /><span className="font-semibold tracking-wide text-lg">Museums</span></div>
